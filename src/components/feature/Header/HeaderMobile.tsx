      ...
      <div className="mx-auto">
        <Link href={"/"} onClick={() => closeMenu()}>
          <div className="flex justify-center">
            <Image src={"/images/logo-header.png"} alt="logo-fhuplus" width={48} height={48} />
          </div>
        </Link>
        <div className="flex justify-center"></div>
        <ul className="flex flex-col space-y-10 items-center font-sans text-lg uppercase mt-12 text-white">
          {ids.map((item) => (
            <li key={item} className="cursor-pointer hover:text-red-500 hover:underline hover:underline-offset-4">
